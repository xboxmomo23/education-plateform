...
export default function ProfesseurEmploiDuTempsPage() {
  ...
  // Charger l'emploi du temps quand la semaine change
  useEffect(() => {
    if (!userId) {
      setLoading(false)
      return
    ...
    const loadTimetable = async () => {
      try {
        ...
        console.log('📅 Chargement emploi du temps professeur:', { userId, weekStartDate })
        
        const response = await timetableApi.getTeacherTimetableForWeek(
          userId, 
          weekStartDate, 
          controller.signal
        )
                              ...
                              <div key={day} className="border border-gray-200 rounded p-1 min-h-[80px] relative">
                                {hourCourses.map(course => (
                                  ...
                                  >
                                    ...
                                    <div className="truncate text-[10px]">{course.class_label}</div>
                    ...
                    <div className="space-y-2">
                      {courses.filter(c => c.status === 'modified').map(course => (
                        <div key={course.id} className="text-sm bg-white p-3 rounded border">
                          <div className="font-medium">{course.subject_name} - {course.class_label}</div>
                ...
                <div className="mt-6 grid grid-cols-3 gap-4">
                  ...
                  <div className="text-center p-4 bg-purple-50 rounded">
                    <div className="text-2xl font-bold text-purple-600">
                      {new Set(courses.map(c => c.class_label)).size}
                    </div>
