...
exports.getStudentGradesSummaryHandler = getStudentGradesSummaryHandler;
exports.getStudentReportHandler = getStudentReportHandler;
exports.getStudentReportDataHandler = getStudentReportDataHandler;
...
// =========================
async function getStudentGradesSummaryHandler(req, res) {
    try {
        if (!req.user) {
        ...
        let termInfo = null;
        if (termId) {
            termInfo = await (0, term_model_1.findTermById)(termId, establishmentId);
            if (!termInfo) {
                res.status(404).json({
                    success: false,
    ...
    `;
        ...
        const gradesResult = await database_1.pool.query(gradesQuery, params);
...
async function getStudentReportHandler(req, res) {
    try {
        ...
        const termId = typeof rawTermId === 'string'
            ? rawTermId
      ...
      WHERE u.id = $1
    `;
        const studentResult = await database_1.pool.query(studentQuery, [studentId]);
        ...
        const reportCard = await (0, reportCard_model_1.findReportCard)(studentId, termId, effectiveEstablishmentId);
      ...
      WHERE id = $1
    `;
        const establishmentResult = await database_1.pool.query(establishmentQuery, [effectiveEstablishmentId]);
        ...
        const term = await (0, term_model_1.findTermById)(termId, effectiveEstablishmentId);
        if (!term) {
    ...
    `;
        const subjectAppreciationsResult = await database_1.pool.query(subjectAppreciationsQuery, [studentId, termId]);
        const subjectAppreciationsMap = new Map();
    ...
    `;
        const teachersResult = await database_1.pool.query(teachersQuery, [studentId, termId, term.start_date, term.end_date]);
      ...
      ORDER BY s.name
    `;
        const gradesResult = await database_1.pool.query(gradesQuery, [studentId, termId, term.start_date, term.end_date]);
        const grades = gradesResult.rows;
      ...
      WHERE student_id = $5
    `;
        const rankResult = await database_1.pool.query(rankQuery, [student.class_id, termId, term.start_date, term.end_date, studentId]);
        ...
        subjectsMap.forEach((subject) => {
            ...
            if (minGrade !== null && minGrade < overallMin)
                overallMin = minGrade;
            if (maxGrade !== null && maxGrade > overallMax)
                overallMax = maxGrade;
            if (classAvg !== null) {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="bulletin_${student.full_name.replace(/\s+/g, '_')}_${term.name.replace(/\s+/g, '_')}.pdf"`);
        doc.pipe(res);
        ...
        if (establishment.address) {
            doc.fontSize(9).font('Helvetica').text(`${establishment.address}`, margin, doc.y + 2, { align: 'center', width: contentWidth });
            doc.text(`${establishment.postal_code} ${establishment.city}`, margin, doc.y, { align: 'center', width: contentWidth });
        }
        doc.moveDown(0.5);
        // Titre avec année scolaire et semestre
        doc.fontSize(10).font('Helvetica-Bold').text(`${student.academic_year}-${student.academic_year + 1} - ${term.name}`, margin, doc.y, { align: 'center', width: contentWidth });
        doc.moveDown(0.3);
        doc.fontSize(12).font('Helvetica-Bold').text(student.class_label.toUpperCase(), margin, doc.y, { align: 'center', width: contentWidth });
        doc.moveDown(0.8);
...
// =========================
async function getStudentReportDataHandler(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, error: 'Non authentifié' });
            return;
    ...
    `;
        const reportCardResult = await database_1.pool.query(reportCardQuery, [studentId, termId]);
