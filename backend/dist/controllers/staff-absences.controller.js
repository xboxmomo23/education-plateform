...
exports.getStaffAbsenceHistoryHandler = getStaffAbsenceHistoryHandler;
exports.exportStaffAbsencesCsvHandler = exportStaffAbsencesCsvHandler;
exports.exportStaffAbsencesPdfHandler = exportStaffAbsencesPdfHandler;
...
function parseHistoryQuery(query) {
    ...
    const status = typeof query.status === 'string' && ALLOWED_STATUS.includes(query.status)
        ? query.status
        : 'all';
    const justified = typeof query.justified === 'string' && ['true', 'false', 'all'].includes(query.justified)
        ? query.justified
        : 'all';
    const sort = typeof query.sort === 'string' && ALLOWED_SORT.includes(query.sort)
        ? query.sort
        : 'date_desc';
    const filters = {
...
async function ensureEstablishmentContext(req, res) {
...
async function validateClassAccess(establishmentId, requestedClassId, assignedClassIds) {
    ...
    const result = await database_1.default.query(`SELECT label FROM classes WHERE id = $1 AND establishment_id = $2 LIMIT 1`, [requestedClassId, establishmentId]);
    return result.rows[0]?.label || null;
}
async function getStaffAbsenceHistoryHandler(req, res) {
    try {
        ...
        const { items, total } = await dashboard_staff_model_1.DashboardStaffModel.getAbsenceHistory(establishmentId, assignedClassIds, filters, pagination.page, pagination.limit);
        return res.json({
            success: true,
            data: {
                items,
...
async function exportStaffAbsencesCsvHandler(req, res) {
    try {
        const establishmentId = await ensureEstablishmentContext(req, res);
        if (!establishmentId) {
            return;
        ...
        const rows = await dashboard_staff_model_1.DashboardStaffModel.getAbsencesForExport(establishmentId, assignedClassIds, filters, 5000);
        const header = [
            'Date',
            'Heure',
            'Élève',
...
function drawPdfHeader(doc, establishmentName, schoolYear, filters, classLabel) {
    const margin = 36;
...
function drawPdfTable(doc, items, margin = 36) {
    const columns = [
    ...
    const drawFooter = () => {
        const prevY = doc.y;
        const currentPageNumber = doc.page?.number || 1;
...
async function exportStaffAbsencesPdfHandler(req, res) {
    try {
        const establishmentId = await ensureEstablishmentContext(req, res);
        if (!establishmentId) {
            return;
        ...
        const [settings, rows, classLabel] = await Promise.all([
            (0, establishmentSettings_model_1.getEstablishmentSettings)(establishmentId),
            dashboard_staff_model_1.DashboardStaffModel.getAbsencesForExport(establishmentId, assignedClassIds, filters, 1000),
...
//# sourceMappingURL=staff-absences.controller.js.map