...
exports.createTermHandler = createTermHandler;
exports.getTermsHandler = getTermsHandler;
exports.getCurrentTermHandler = getCurrentTermHandler;
exports.getTermByIdHandler = getTermByIdHandler;
exports.updateTermHandler = updateTermHandler;
exports.deleteTermHandler = deleteTermHandler;
 ...
 */
async function createTermHandler(req, res) {
    try {
        if (!req.user) {
        ...
        const term = await (0, term_model_1.createTerm)({
            academicYear,
            name,
            startDate: start,
            endDate: end,
 ...
 */
async function getTermsHandler(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, error: 'Non authentifié' });
            return;
        ...
        const terms = await (0, term_model_1.findTerms)(filters);
 ...
 */
async function getCurrentTermHandler(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, error: 'Non authentifié' });
            return;
        ...
        const term = await (0, term_model_1.getCurrentTerm)(establishmentId);
        if (!term) {
            res.json({
                success: true,
                data: null,
 ...
 */
async function getTermByIdHandler(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, error: 'Non authentifié' });
            return;
        ...
        const term = await (0, term_model_1.findTermById)(id, establishmentId);
        if (!term) {
            res.status(404).json({
                success: false,
 ...
 * Modifie une période
 */
async function updateTermHandler(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, error: 'Non authentifié' });
            return;
        ...
        const updatedTerm = await (0, term_model_1.updateTerm)(id, updateData, establishmentId);
        if (!updatedTerm) {
            res.status(404).json({
                success: false,
                error: 'Période non trouvée',
 ...
 */
async function deleteTermHandler(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, error: 'Non authentifié' });
            return;
        ...
        const deleted = await (0, term_model_1.deleteTerm)(id, establishmentId);
        if (!deleted) {
            res.status(404).json({
                success: false,
                error: 'Période non trouvée',
