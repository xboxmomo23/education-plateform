...
exports.getAdminPerformanceMetrics = getAdminPerformanceMetrics;
const database_1 = __importDefault(require("../config/database"));
async function getAdminKpis(establishmentId, date) {
    const countsQuery = `
    SELECT
      COUNT(*) FILTER (
        WHERE role = 'student'
  ...
  `;
    const countsResult = await database_1.default.query(countsQuery, [establishmentId]);
  ...
  `;
    const absencesResult = await database_1.default.query(absencesQuery, [establishmentId, date]);
...
async function getAdminPerformanceMetrics(establishmentId, options) {
  ...
  `;
    const result = await database_1.default.query(query, [establishmentId, termId, classId]);
    const row = result.rows[0];
    const classes = row?.classes || [];
    const topStudents = row?.top_students || [];
    const classLogUnique = new Set(classes.map((cls) => cls.class_id)).size;
    const studentLogUnique = new Set(topStudents.map((student) => student.student_id)).size;
...
//# sourceMappingURL=dashboard-admin.model.js.map