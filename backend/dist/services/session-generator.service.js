...
exports.generateSessionsForDay = generateSessionsForDay;
exports.generateSessionsForWeek = generateSessionsForWeek;
exports.generateSessionsForMonth = generateSessionsForMonth;
exports.generateSessionsForClass = generateSessionsForClass;
exports.autoGenerateIfNeeded = autoGenerateIfNeeded;
exports.dailySessionGeneration = dailySessionGeneration;
 ...
 */
async function sessionExists(courseId, sessionDate, startTime) {
    const query = `
  ...
  `;
    const result = await database_1.default.query(query, [courseId, sessionDate, startTime]);
 ...
 */
async function generateSessionsForDay(date) {
    const result = {
        generated: 0,
        skipped: 0,
        errors: [],
    ...
    try {
        ...
        const entries = await timetable_model_1.TimetableModel.getEntriesByDayOfWeek(dayOfWeek, weekType);
        for (const entry of entries) {
            try {
                // Vérifier si la session existe déjà
                const exists = await sessionExists(entry.course_id, dateStr, entry.start_time);
                if (exists) {
                    result.skipped++;
                    continue;
                }
                // Créer la session
                await database_1.default.query(`INSERT INTO attendance_sessions (
 ...
 */
async function generateSessionsForWeek(startDate) {
    const result = {
        generated: 0,
        skipped: 0,
        errors: [],
    ...
    console.log(`[Session Generator] Semaine: ${result.generated} générées, ${result.skipped} ignorées`);
    return result;
}
/**
 * Générer les sessions pour un mois
 */
async function generateSessionsForMonth(year, month) {
    const result = {
        generated: 0,
        skipped: 0,
        errors: [],
    ...
    console.log(`[Session Generator] Mois ${month + 1}/${year}: ${result.generated} générées, ${result.skipped} ignorées`);
    return result;
}
/**
 * Générer les sessions pour une classe spécifique
 */
async function generateSessionsForClass(classId, startDate, endDate) {
    const result = {
        generated: 0,
        skipped: 0,
        errors: [],
    ...
    try {
        ...
        console.log(`[Session Generator] Classe ${classId}: ${result.generated} générées, ${result.skipped} ignorées`);
        return result;
    }
    catch (error) {
        console.error('[Session Generator] Erreur:', error);
 ...
 */
async function autoGenerateIfNeeded(date) {
    try {
        const dateStr = date.toISOString().split('T')[0];
    ...
    `;
        const checkResult = await database_1.default.query(checkQuery, [dateStr]);
 ...
 */
async function dailySessionGeneration() {
    try {
...
*/ 
//# sourceMappingURL=session-generator.service.js.map