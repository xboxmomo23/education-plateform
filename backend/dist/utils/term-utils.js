...
exports.findTermIdByDate = findTermIdByDate;
exports.createEvaluationWithAutoTerm = createEvaluationWithAutoTerm;
exports.assignMissingTermIds = assignMissingTermIds;
 ...
 */
async function findTermIdByDate(evalDate, establishmentId, academicYear) {
  ...
  `;
    try {
        const result = await database_1.pool.query(query, [establishmentId, evalDate, year]);
 ...
 */
async function createEvaluationWithAutoTerm(data) {
 ...
 */
async function assignMissingTermIds(establishmentId) {
  ...
  `;
    const evaluations = await database_1.pool.query(evaluationsQuery, [establishmentId]);
    let updatedCount = 0;
    for (const evaluation of evaluations.rows) {
        const termId = await findTermIdByDate(new Date(evaluation.eval_date), establishmentId, evaluation.academic_year);
        if (termId) {
            await database_1.pool.query('UPDATE evaluations SET term_id = $1 WHERE id = $2', [termId, evaluation.id]);
            updatedCount++;
        }
    }
