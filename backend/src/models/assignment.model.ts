...
export const AssignmentModel = {
  ...
  async create(data: CreateAssignmentData): Promise<Assignment> {
    ...
    try {
      ...
      const enrichedAssignment = await AssignmentModel.getById(assignment.id);
    ...
    try {
      ...
      if (data.course_id) {
        ...
        if (newClassId !== oldClassId) {
          ...
          await client.query(
            `INSERT INTO assignment_targets (assignment_id, class_id) VALUES ($1, $2)`,
            [assignmentId, newClassId]
          );
        }
      }
      ...
      // 4. Récupérer le devoir enrichi
      const enrichedAssignment = await AssignmentModel.getById(assignmentId);
      return enrichedAssignment!;
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
  ...
  async getForStudent(
    studentId: string,
    filters: { subjectId?: string; fromDueAt?: string; toDueAt?: string } = {}
  ): Promise<Assignment[]> {
    ...
    const params: any[] = [classId, establishmentId];
    let paramIndex = 3;
  ...
  async verifyCourseOwnership(
    ...
    const result = await pool.query(query, [courseId, teacherId, establishmentId]);
   ...
   */
  async getUserEstablishment(userId: string): Promise<string | null> {
