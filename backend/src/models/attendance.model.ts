...
export const AttendanceModel = {
   ...
   */
  async getTeacherWeekCourses(
    teacherId: string,
    weekStartDate: string,
    establishmentId?: string
  ): Promise<TeacherWeekCourse[]> {
    const query = `
      SELECT 
  ...
  async getOrCreateSession(
    ...
    const createResult = await pool.query(createQuery, [instanceId, userId]);
    const sessionId = createResult.rows[0].session_id;

    // Initialiser les enregistrements de présence
    await pool.query('SELECT init_attendance_records($1, $2)', [sessionId, userId]);
   ...
   */
  async getSessionById(sessionId: string): Promise<AttendanceSessionWithDetails> {
    const query = `
      SELECT 
        ases.*,
        cl.label AS class_label,
   ...
   */
  async getSessionByInstanceId(instanceId: string): Promise<AttendanceSessionWithDetails | null> {
    const query = `
      SELECT 
        ases.*,
        cl.label AS class_label,
   ...
   */
  async getRecentTeacherAbsences(
    teacherId: string,
    options: { date: string; days: number; limit: number }
  ) {
    ...
    const result = await pool.query(query, [teacherId, date, days, limit]);
    return result.rows;
  },
   ...
   */
  async getSessionStudents(sessionId: string): Promise<StudentForAttendance[]> {
    ...
    const result = await pool.query(query, [sessionId, classId]);
    return result.rows;
  },
   ...
   */
  async getSessionsStatusByInstanceIds(
    teacherId: string,
    instanceIds: string[]
  ): Promise<
    Array<{
  ...
  > {
    ...
    const result = await pool.query(query, [teacherId, instanceIds]);
    return result.rows;
  },
  ...
  async canAccessSession(
    ...
    if (role === 'staff') {
      const staffCheck = await pool.query(
        'SELECT 1 FROM class_staff WHERE user_id = $1 AND class_id = $2',
