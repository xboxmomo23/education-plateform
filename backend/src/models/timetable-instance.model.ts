...
export const TimetableInstanceModel = {
   ...
   */
  async generateFromTemplate(classId: string, weekStartDate: string, createdBy: string): Promise<number> {
    const result = await pool.query(
      'SELECT generate_instances_from_template($1, $2, $3) as count',
      [classId, weekStartDate, createdBy]
    );
