...
export interface TimetableOverride {
  ...
  override_date: string; // Format: YYYY-MM-DD
...
export const TimetableOverrideModel = {
  /**
   * Créer une exception
   */
  async create(data: CreateOverrideData): Promise<TimetableOverride> {
    const query = `
   ...
   */
  async getOverridesForWeek(classId: string, weekStartDate: string): Promise<TimetableOverrideWithDetails[]> {
    const query = `
      SELECT 
        o.*,
   ...
   */
  async getById(overrideId: string): Promise<TimetableOverrideWithDetails | null> {
    const query = `
      SELECT 
        o.*,
        s.name as template_subject_name,
    ...
    const result = await pool.query(query, [overrideId]);
   ...
   */
  async update(overrideId: string, data: Partial<CreateOverrideData>): Promise<TimetableOverride> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
   ...
   */
  async existsForEntryAndDate(templateEntryId: string, overrideDate: string): Promise<boolean> {
    const result = await pool.query(
      'SELECT 1 FROM timetable_overrides WHERE template_entry_id = $1 AND override_date = $2',
      [templateEntryId, overrideDate]
    );
