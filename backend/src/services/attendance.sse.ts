...
export function emitAttendanceUpdate(
  ...
  console.log(
    `[SSE] Mise à jour émise pour la session ${sessionId} (${sessionClients.length} clients)`
  );
}
 ...
 */
export function getConnectedClientsCount(sessionId?: string): number {
  if (sessionId) {
 ...
 */
export function sseStreamHandler(req: any, res: Response): void {
